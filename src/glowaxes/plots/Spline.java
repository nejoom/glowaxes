 ...
 * The Class Spline.
 */
public class Spline extends ALinePlotter {

    // Define a static logger variable so that it references the
    // Logger instance named after class.
    /** The logger. */
    private static Logger logger = Logger.getLogger(Spline.class.getName());
    ...
    public Element drawSeries(Series values) {
        ...
        for (int i = 0; i < values.getSize(); i++) {
            ...
            double x =
                    getData().getParentArea().getXAxis().getSVGOffset(myValue);

            double y =
                    getData().getParentArea().getYAxis().getSVGOffset(myValue);
        ...
        path.setAttribute("d", BezierControlPoints.getPath(mySeries));
